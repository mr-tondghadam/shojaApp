...
const HCardProduct = ({ category = "", heading = "", data = [] }) => {
  ...
  return (
    <div className='container overflow-y-hidden mx-auto px-1 md:px-4 gap-2 my-6 relative'>
      <h2
        data-aos='fade-up'
      ...
      {loading ? (
        ...
        <div
          className='flex items-center scrollbar-hide overflow-x-scroll transition-allpx-2'
          ref={scrollElement}>
          <button
          ...
          {data?.map((el, index) => (
            <Link
              ...
              to={`#`}
              key={el._id}
              className='group hover:bg-slate-200 bg-white overflow-hidden flex relative items-center min-w-[200px] m-1 md:m-3 md:min-w-[350px] max-w-[300px] h-40 md:max-w-[250px] rounded-md p-2  shadow-lg'>
              <div className='absolute rounded-md text-xs md:text-sm ms-6 right-0 top-2 md:top-6 bg-red-500 px-1 md:px-2'>
                <span className='relative mt-5 z-50 text-xs md:text-sm md:font-bold self-end text-white shadow-lg'>
                  {Math.floor(
                    100 - (Number(el?.sellingPrice) / Number(el?.price)) * 100
                  )}{" "}
                  درصد تخفیف
                </span>
                <span className='absolute top-1 hidden md:block bg-red-500 w-[12px] z-10 -start-1 rotate-45 md:h-[12px]'></span>
              </div>
              <div className='h-30 md:h-40 flex items-center justify-center max-w-[80px] md:max-w-[170px]'>
                <img
                  src={el.productImage}
                  alt={el?.productName}
